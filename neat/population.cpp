...
Population::Population(const char *filename) {
	...
	if (!iFile) {
        ...
        while (iFile.getline(curline, sizeof(curline)))
		{
...
bool Population::print_to_file_by_species(std::ofstream& outFile) {
...
bool Population::epoch(int generation) {
	...
	*/
    ...
    neatDebugger()<<"Number of Species: "<<num_species<<std::endl;
    neatDebugger()<<"compat_thresh: "<<compat_threshold<<std::endl;
    ...
    neatDebugger()<<"Generation "<<generation<<": "<<"overall_average = "<<overall_average<<std::endl;
	...
	for(curspecies=sorted_species.begin();curspecies!=sorted_species.end();++curspecies) {
        ...
        neatDebugger()<<"orig fitness of Species"<<(*curspecies)->id<<"(Size "<<(*curspecies)->organisms.size()<<"): "<<(*((*curspecies)->organisms).begin())->orig_fitness<<" last improved "<<((*curspecies)->age-(*curspecies)->age_of_last_improvement)<<std::endl;
	}
	...
	if (((*(((*curspecies)->organisms).begin()))->orig_fitness)>
		highest_fitness) {
			...
			highest_last_changed=0;
            neatDebugger()<<"NEW POPULATION RECORD FITNESS: "<<highest_fitness<<std::endl;
		}
	else {
		++highest_last_changed;
        neatDebugger()<<highest_last_changed<<" generations since last population fitness record: "<<highest_fitness<<std::endl;
	}
